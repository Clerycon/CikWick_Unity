...
public class CatController : MonoBehaviour
{
    ...
    [SerializeField] private float _maxDestinationAttempts;
    ...
    private void SetChaseMovement()
    {
        ...
        _catStateController.ChangeState(ECatState.Running);

        if(Vector3.Distance(_playerTransform.position, transform.position) <= _chaseDistance &&  _isChasing)
        {
            OnCatCatched?.Invoke();
            _catStateController.ChangeState(ECatState.Attacking);
            _isChasing = false;
        }
    }
    ...
    private void SetPatrolMovement()
    {
        ...
        if(!_catAgent.pathPending && _catAgent.remainingDistance <= _catAgent.stoppingDistance)
        {
            if (!_isWaiting)
            {
                _isWaiting = true;
                _timer = _waitTime;
                _catStateController.ChangeState(ECatState.Idle);
            }
        }
        ...
        if (_isWaiting)
        {
            ...
            if(_timer <= 0f)
            {
                _isWaiting = false;
                SetRandomDestination();
                _catStateController.ChangeState(ECatState.Walking);    
            }
        }
    }
